...
export class Span<Data = any> {
  ...
  static slice(spans: readonly Span[], from: number, to: number) {
    ...
    for (let i = 0, off = 0; off < to; i++) {
      ...
      const overlap = Math.min(to, end) - Math.max(from, off);
  ...
  static join<Data>(
    ...
    const combined = combine(a[a.length - 1].data, b[0].data);
...
export class Change<Data = any> {
  ...
  static merge<Data>(
    ...
    for (
      let iX = 0, iY = 0, curX: Change<Data> | null = x[0], curY: Change<Data> | null = y[0];
      ;
      ...
      if (!curX && !curY) {
        ...
        const off = iY ? y[iY - 1].toB - y[iY - 1].toA : 0;
        result.push(
          off == 0
            ? curX
            : new Change(
        ...
        const off = iX ? x[iX - 1].toB - x[iX - 1].toA : 0;
        result.push(
          off == 0
            ? curY
            : new Change(
        ...
        for (;;) {
          const nextX = !curX ? 2e8 : pos >= curX.fromB ? curX.toB : curX.fromB;
          const nextY = !curY ? 2e8 : pos >= curY.fromA ? curY.toA : curY.fromA;
          const next = Math.min(nextX, nextY);
          const inX = curX && pos >= curX.fromB,
